...
def setup_logging(log_dir: str = "logs") -> logging.Logger:
    ...
    # Setup main logger
    logger = logging.getLogger('PSAT_Generator')
    ...
    # Log initialization
    logger.info("=" * 80)
    logger.info("PSAT Math Task Generator - Logging Initialized")
...
class ConcurrentMathTaskGenerator:
    def __init__(self, project_id: str = "studyhall-dev-383420", max_workers: int = 1):
        ...
        self.logger.info(f"Initializing PSAT Math Task Generator")
...
After you have finished thinking, provide your response which MUST ONLY BE the final, clean, valid JSON array containing exactly {num_to_generate} problems. Do not include any of your thought process, notes, or any other text before or after the JSON array. The response must start with `[` and end with `]`."""
    ...
    async def generate_tasks_for_skill(self, skill: str, examples: List[Dict[str, Any]], 
        ...
        all_generated = []
        examples_for_prompt = self.prepare_examples_for_prompt(examples, 10)
...
- Ensure all LaTeX expressions are properly escaped for valid JSON format"""
    ...
    async def generate_additional_tasks_by_difficulty(self, skill: str, examples: List[Dict[str, Any]],
        ...
        for difficulty, target_count in tasks_to_generate.items():
            ...
            self.logger.info(f"n[{skill}] Generating {target_count} additional tasks for difficulty: {difficulty}")
            ...
            try:
                ...
                if skill_name:
                    ...
                    if all(tasks_by_difficulty.get(diff, 0) >= 90 for diff in ["Easy", "Medium", "Hard"]):
                        ...
                        self.logger.info(f" ✅ Complete: {skill_name} ({dict(tasks_by_difficulty)})")
                    else:
                        incomplete_skills.add(skill_name)
                        self.logger.warning(f" ⚠️ Incomplete: {skill_name} ({dict(tasks_by_difficulty)})")
            except Exception as e:
                self.logger.info(f"arning: Could not read {file}: {e}")
            ...
            try:
                ...
                if test_mode:
                    ...
                    if all(count <= 0 for count in tasks_to_generate.values()):
                        self.logger.warning(f"{skill}] All difficulties have sufficient tasks (≥90), skipping generation")
                        return skill, 0
    ...
    async def process_all_skills(self, input_csv: str, tasks_per_difficulty: int = 100, 
        ...
        self.logger.info(f"asks per difficulty level: {tasks_per_difficulty}")
        self.logger.info(f"otal tasks per skill: {tasks_per_difficulty * 3} (assuming Easy, Medium, Hard)")
        self.logger.info(f"est mode: {test_mode} ({test_tasks} total tasks per skill in test mode)" if test_mode else "Test mode: {test_mode}")
        self.logger.info(f"est mode: {test_mode}")
        self.logger.info(f"nly remaining skills: {only_remaining}")
        self.logger.info(f"ax concurrent workers: {self.max_workers}")
        pass  # Empty print removed
    ...
    def check_existing_tasks_by_difficulty(self, skill: str) -> Dict[str, int]:
        ...
        try:
            ...
            self.logger.error(f"arning: Error reading {matching_files[0]}: {e}")
            return {"Easy": 100, "Medium": 100, "Hard": 100}
...
async def main():
    # Configuration
    INPUT_CSV = "PSAT_NMSQT & PSAT 10 Questions - PSAT_Math_No_Graph.csv"  # Your input CSV file
